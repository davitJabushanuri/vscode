...
const bundleVSCodeTask = task.define(
	"bundle-vscode",
	task.series(
		...
		optimize.bundleTask({
			...
			esm: {
				src: "out-build",
				entryPoints: [...vscodeEntryPoints, ...bootstrapEntryPoints],
				resources: vscodeResources,
				skipTSBoilerplateRemoval: (entryPoint) =>
					entryPoint === "vs/code/electron-browser/workbench/workbench",
			},
		})
	)
);
...
const minifyVSCodeTask = task.define(
	"minify-vscode",
	task.series(
		bundleVSCodeTask,
...
function packageTask(
	...
	const task = () => {
		...
		const platformSpecificBuiltInExtensionsExclusions =
		...
		const productJsonStream = gulp
			...
			.pipe(
				json({ commit, date: readISODate("out-build"), checksums, version })
			)
			.pipe(
				es.through(function (file) {
		...
		const api = gulp
			.src("src/vscode-dts/vscode.d.ts")
			.pipe(rename("out/vscode-dts/vscode.d.ts"));

		const telemetry = gulp.src(".build/telemetry/**", {
		...
		const deps = gulp
			.src(dependenciesSrc, { base: ".", dot: true })
			.pipe(
				filter([
					"**",
					`!**/${config.version}/**`,
		...
		if (platform === "win32") {
			...
			const shortcut = gulp
				.src("resources/darwin/bin/code.sh")
				.pipe(replace("@@APPNAME@@", product.applicationName))
				.pipe(rename("bin/code"));
			const policyDest = gulp
				.src(".build/policies/darwin/**", { base: ".build/policies/darwin" })
		...
		if (platform === "linux") {
			result = es.merge(
				result,
				gulp
					.src("resources/completions/bash/code", { base: "." })
					.pipe(replace("@@APPNAME@@", product.applicationName))
					.pipe(
						rename(function (f) {
			...
			result = es.merge(
				result,
				gulp
					.src("resources/completions/zsh/_code", { base: "." })
					.pipe(replace("@@APPNAME@@", product.applicationName))
					.pipe(
						rename(function (f) {
		...
		if (platform === "win32") {
			...
			result = es.merge(
				result,
				gulp
					.src("resources/win32/bin/code.cmd", { base: "resources/win32" })
			...
			result = es.merge(
				result,
				gulp
					.src("resources/win32/bin/code.sh", { base: "resources/win32" })
					.pipe(replace("@@NAME@@", product.nameShort))
			...
			if (quality === "stable" || quality === "insider") {
				...
				result = es.merge(
					result,
					gulp
						.src("resources/win32/appx/AppxManifest.xml", { base: "." })
						...
						.pipe(
							replace("@@AppxPackageDescription@@", product.win32NameVersion)
						)
						.pipe(replace("@@ApplicationIdShort@@", product.win32RegValueName))
						.pipe(replace("@@ApplicationExe@@", product.nameShort + ".exe"))
						.pipe(
							replace(
								"@@FileExplorerContextMenuID@@",
								quality === "stable" ? "OpenWithCode" : "OpenWithCodeInsiders"
							)
						)
						.pipe(
							replace(
								"@@FileExplorerContextMenuCLSID@@",
								product.win32ContextMenu[arch].clsid
							)
						)
						.pipe(
							replace(
								"@@FileExplorerContextMenuDLL@@",
								`${
...
function patchWin32DependenciesTask(destinationFolderName) {
	...
	return async () => {
		const deps = await glob("**/*.{exe,dll}", {
			cwd,
			ignore: "extensions/node_modules/@parcel/watcher/**",
		});
...
const innoSetupConfig = {
	"zh-cn": {
		codePage: "CP936",
		defaultInfo: { name: "Simplified Chinese", id: "$0804" },
	},
	"zh-tw": {
		codePage: "CP950",
		defaultInfo: { name: "Traditional Chinese", id: "$0404" },
	},
...
gulp.task(
	task.define(
		...
		task.series(coreCI, compileAllExtensionsBuildTask, function () {
...
gulp.task("vscode-translations-import", function () {
	...
	return es.merge(
		[...i18n.defaultLanguages, ...i18n.extraLanguages].map((language) => {
