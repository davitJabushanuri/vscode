...
export async function collectWorkspaceStats(
	...
	const configFilePatterns: ConfigFilePatterns[] = [
		...
		{ tag: "kodikrules", filePattern: /^\.kodikrules$/i },
		{
			tag: "kodikrules-dir",
			filePattern: /\.md$/i,
			relativePathPattern: /^\.kodikrules$/i,
		},
...
export function getMachineInfo(): IMachineInfo {
	const machineInfo: IMachineInfo = {
		...
		memory: `${(osLib.totalmem() / ByteSize.GB).toFixed(2)}GB (${(
			osLib.freemem() / ByteSize.GB
		).toFixed(2)}GB free)`,
...
export class DiagnosticsService implements IDiagnosticsService {
	...
	private formatEnvironment(info: IMainProcessDiagnostics): string {
		const output: string[] = [];
		output.push(
			`Version:          ${this.productService.nameShort} ${
		...
		output.push(
			`Memory (System):  ${(osLib.totalmem() / ByteSize.GB).toFixed(2)}GB (${(
				osLib.freemem() / ByteSize.GB
			).toFixed(2)}GB free)`
		);
		if (!isWindows) {
	...
	private formatWorkspaceStats(workspaceStats: WorkspaceStats): string {
		...
		if (workspaceStats.launchConfigFiles.length > 0) {
			...
			workspaceStats.launchConfigFiles.forEach((each) => {
				const item =
					each.count > 1 ? ` ${each.name}(${each.count})` : ` ${each.name}`;
				line += item;
			});
			output.push(line);
		}
	...
	private expandGPUFeatures(gpuFeatures: any): string {
		...
		return Object.keys(gpuFeatures)
			.map(
				(feature) =>
					`${feature}:  ${" ".repeat(longestFeatureName - feature.length)}  ${
						gpuFeatures[feature]
					}`
			)
			.join("\n                  ");
	...
	private formatProcessItem(
		...
		output.push(
			`${item.load.toFixed(0).padStart(5, " ")}\t${(memory / ByteSize.MB)
				.toFixed(0)
				.padStart(6, " ")}\t${item.pid.toFixed(0).padStart(6, " ")}\t${name}`
		);
