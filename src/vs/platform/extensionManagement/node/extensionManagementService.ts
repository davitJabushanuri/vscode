	...
	implements INativeServerExtensionManagementService
{
	...
	constructor(
		...
		this.extensionsScanner = this._register(
			instantiationService.createInstance(ExtensionsScanner, (extension) =>
				extensionLifecycle.postUninstall(extension)
			)
		);
	...
	async zip(extension: ILocalExtension): Promise<URI> {
		...
		const location = await zip(
			joinPath(this.extensionsDownloader.extensionsDownloadDir, generateUuid())
				.fsPath,
			files
		);
		...
		try {
			...
			if (
				...
				throw new Error(
					nls.localize(
						"incompatible",
						"Unable to install extension '{0}' as it is not compatible with Architech '{1}'.",
						extensionId,
						this.productService.version
					)
				);
			...
			throw toExtensionManagementError(
				new Error(`Unknown error while installing extension ${extensionId}`)
			);
		} finally {
			await cleanup();
		}
	...
	async installFromLocation(
		...
		const local = await this.extensionsScanner.scanUserExtensionAtLocation(
			location
		);
	...
	protected moveExtension(
		extension: ILocalExtension,
		fromProfileLocation: URI,
		toProfileLocation: URI,
		metadata: Partial<Metadata>
	): Promise<ILocalExtension> {
		return this.extensionsScanner.moveExtension(
			extension,
			fromProfileLocation,
			toProfileLocation,
			metadata
	...
	protected removeExtension(
		...
		return this.extensionsScanner.removeExtension(
	...
	copyExtensions(
		...
		return this.extensionsScanner.copyExtensions(
			fromProfileLocation,
			toProfileLocation,
			{ version: this.productService.version, date: this.productService.date }
		);
	}
		...
		return this.instantiationService.createInstance(
			...
			(operation, token) => {
				...
				if (!promise) {
					...
					promise.finally(() =>
						this.extractingGalleryExtensions.delete(extensionKey.toString())
					);
				}
				return promise;
			},
	...
	private async downloadExtension(
		...
		const shouldRequireSignature = shouldRequireRepositorySignatureFor(
			extension.private,
		...
		for (const resource of e.rawAdded) {
			...
			if (
				this.uriIdentityService.extUri
					.basename(resource)
					.endsWith(DELETED_FOLDER_POSTFIX)
			) {
				continue;
			}
			...
			const extension =
				await this.extensionsScanner.scanUserExtensionAtLocation(resource);
	...
	private async addExtensionsToProfile(
		...
		await this.extensionsScanner.unsetExtensionsForRemoval(
			...localExtensions.map((extension) => ExtensionKey.create(extension))
		);
...
export class ExtensionsScanner extends Disposable {
	...
	constructor(
		...
		super();
		this.obsoletedResource = joinPath(
			this.extensionsScannerService.userExtensionsLocation,
			".obsolete"
		);
	...
	async scanExtensions(
		...
		try {
			...
			const userScanOptions: UserExtensionsScanOptions = {
				includeInvalid: true,
				profileLocation,
				productVersion,
				language,
			...
			if (type === null || type === ExtensionType.System) {
				let scanAllExtensionsPromise =
					this.scanAllExtensionPromise.get(cacheKey);
				if (!scanAllExtensionsPromise) {
					scanAllExtensionsPromise = this.extensionsScannerService
						.scanAllExtensions({ language }, userScanOptions)
				...
				let scanUserExtensionsPromise =
					this.scanUserExtensionsPromise.get(cacheKey);
			...
			return await Promise.all(
				scannedExtensions.map((extension) => this.toLocalExtension(extension))
			);
		} catch (error) {
			throw toExtensionManagementError(
				error,
	...
	async scanAllUserExtensions(): Promise<ILocalExtension[]> {
		try {
			const scannedExtensions =
				await this.extensionsScannerService.scanAllUserExtensions();
			return await Promise.all(
				scannedExtensions.map((extension) => this.toLocalExtension(extension))
			);
		} catch (error) {
	...
	async extractUserExtension(
		...
		try {
			...
			try {
				await this.extensionsScannerService.updateManifestMetadata(
					tempLocation,
					metadata
				);
			} catch (error) {
				...
				if (error.code === "ENOTEMPTY") {
					...
					this.logService.info(
						`Rename failed because of ${getErrorMessage(
							error
						)}. Deleted from extracted location`,
						tempLocation
					);
					throw error;
				}
	...
	async setExtensionsForRemoval(...extensions: IExtension[]): Promise<void> {
		...
		const extensionKeys: ExtensionKey[] = extensionsToRemove.map((e) =>
	...
	async unsetExtensionsForRemoval(
	...
	async copyExtension(
		...
		if (target) {
			if (
				...
				await this.extensionsProfileScannerService.removeExtensionsFromProfile(
					[targetExtension.identifier],
					toProfileLocation
				);
				await this.extensionsProfileScannerService.addExtensionsToProfile(
					[[extension, { ...target.metadata, ...metadata }]],
	...
	private async deleteExtensionFromLocation(
		...
		const renamedLocation = this.uriIdentityService.extUri.joinPath(
			...
			`${this.uriIdentityService.extUri.basename(location)}.${hash(
				generateUuid()
			).toString(16)}${DELETED_FOLDER_POSTFIX}`
		);
			...
			if (updateFn) {
				...
				if (Object.keys(removed).length) {
					await this.fileService.writeFile(
						this.obsoletedResource,
						VSBuffer.fromString(JSON.stringify(removed))
					);
				} else {
					try {
		...
		try {
			if (profileLocation) {
				...
				const scannedExtension = scannedExtensions.find((e) =>
					this.uriIdentityService.extUri.isEqual(e.location, location)
				);
				if (scannedExtension) {
					return await this.toLocalExtension(scannedExtension);
				}
	...
	private async initializeExtensionSize(): Promise<void> {
		...
		await Promise.all(
			extensions.map(async (extension) => {
				...
				if (
					isDefined(extension.metadata?.installedTimestamp) &&
	...
	private async deleteExtensionsMarkedForRemoval(): Promise<void> {
		...
		this.logService.debug(
			`Deleting extensions marked as removed:`,
			Object.keys(removed)
		);
		...
		const toRemove = extensions.filter(
			(e) =>
				e.installedTimestamp /* Installed by System */ &&
				removed[ExtensionKey.create(e).toString()]
		);
		await Promise.allSettled(
			toRemove.map((e) => this.deleteExtension(e, "marked for removal"))
		);
	}
	...
	implements IInstallExtensionTask
{
	...
	protected async doRun(token: CancellationToken): Promise<ILocalExtension> {
		...
		const metadata: Metadata = {
			...
			isMachineScoped:
				this.options.isMachineScoped || existingExtension?.isMachineScoped,
		...
		if (this.source instanceof URI) {
			if (existingExtension) {
				if (
					...
					try {
						await this.extensionsScanner.deleteExtension(
							existingExtension,
							"existing"
						);
					} catch (e) {
						throw new Error(
							nls.localize(
								"restartCode",
								"Please restart Architech before reinstalling {0}.",
			...
			const existingWithSameVersion = await this.unsetIfRemoved(
				this.extensionKey
			);
	...
	private async unsetIfRemoved(
		...
		const [removed] = await this.extensionsScanner.unsetExtensionsForRemoval(
			extensionKey
		);
		if (removed) {
			this.logService.info(
				"Removed the extension from removed list:",
				extensionKey.id
			);
			const userExtensions =
				await this.extensionsScanner.scanAllUserExtensions();
	...
	private async updateMetadata(
		...
		try {
			let [galleryExtension] = await this.galleryService.getExtensions(
				[{ id: extension.identifier.id, version: extension.manifest.version }],
				token
			);
			if (!galleryExtension) {
	...
	implements IUninstallExtensionTask
{
	...
	protected doRun(token: CancellationToken): Promise<void> {
		return this.extensionsProfileScannerService.removeExtensionsFromProfile(
