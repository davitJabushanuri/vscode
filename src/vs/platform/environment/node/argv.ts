...
export const OPTIONS: OptionDescriptions<Required<NativeParsedArgs>> = {
	chat: {
		...
		options: {
			_: {
				type: "string[]",
				description: localize("prompt", "The prompt to use as chat."),
			},
			mode: {
				type: "string",
				cat: "o",
	...
	telemetry: {
		...
		description: localize(
			"telemetry",
			"Shows all telemetry events which Architech collects."
		),
	},
	...
	"startup-experiment-group": {
		type: "string",
		cat: "t",
		args: "control|maximizedChat|splitEmptyEditorChat|splitWelcomeChat",
		description: localize(
...
export function parseArgs<T>(
	...
	const firstPossibleCommand = args.find(
		(a, i) =>
			a.length > 0 &&
			a[0] !== "-" &&
	...
	if (command && firstPossibleCommand) {
		...
		const reporter = errorReporter.getSubcommandReporter
			? errorReporter.getSubcommandReporter(firstPossibleCommand)
			: undefined;
...
function wrapText(text: string, columns: number): string[] {
	...
	while (text.length) {
		let index =
			text.length < columns ? text.length : text.lastIndexOf(" ", columns);
		if (index === 0) {
			index = columns;
		}
...
export function buildHelpMessage(
	...
	version: string,
	options: OptionDescriptions<any>,
	capabilities?: { noPipe?: boolean; noInputFiles?: boolean; isChat?: boolean }
): string {
	const columns = (process.stdout.isTTY && process.stdout.columns) || 80;
	const inputFiles = capabilities?.noInputFiles
		? ""
		: capabilities?.isChat
		? ` [${localize("cliPrompt", "prompt")}]`
	...
	help.push("");
	help.push(
		`${localize("usage", "Usage")}: ${executableName}${subcommand} [${localize(
			"options",
			"options"
		)}]${inputFiles}`
	);
...
export function buildStdinMessage(
	executableName: string,
	isChat?: boolean
): string {
	let example: string;
