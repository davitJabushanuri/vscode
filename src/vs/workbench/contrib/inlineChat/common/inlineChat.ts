...
Registry.as<IConfigurationRegistry>(
	...
	properties: {
		...
		[InlineChatConfigKeys.notebookAgent]: {
			markdownDescription: localize(
				"notebookAgent",
				"Enable agent-like behavior for inline chat widget in notebooks. Depends on the `#inlineChat.enableV2#` setting being enabled."
			),
			default: false,
			type: "boolean",
			tags: ["experimental"],
...
export const CTX_INLINE_CHAT_HAS_AGENT2 = new RawContextKey<boolean>(
	"inlineChatHasEditsAgent",
	false,
	localize(
		"inlineChatHasEditsAgent",
		"Whether an agent for inliine for interactive editors exists"
	)
);
export const CTX_INLINE_CHAT_HAS_KODIK_AGENT = new RawContextKey<boolean>(
	"inlineChatHasKodikAgent",
	false,
	localize(
		"inlineChatHasKodikAgent",
		"Whether the Kodik agent is the active inline chat agent"
	)
);
