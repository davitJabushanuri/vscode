...
registerAction2(
	class extends Action2 {
			...
			super({
				...
				metadata: {
					description: localize2(
						...
						"Opens a Walkthrough to help you get started in Architech."
					),
				},
			});
		}
		...
		public run(
			...
			optionsOrToSide:
				| { toSide?: boolean; inactive?: boolean }
				| boolean
				| undefined
		) {
			...
			const toSide =
				typeof optionsOrToSide === "object"
					? optionsOrToSide.toSide
					: optionsOrToSide;
			const inactive =
				typeof optionsOrToSide === "object" ? optionsOrToSide.inactive : false;
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor) {
			const editorService = accessor.get(IEditorService);
			const options: GettingStartedEditorOptions = {
				selectedCategory: "NewWelcomeExperience",
				forceReload: true,
		...
		"workbench.startupEditor": {
			...
			enumDescriptions: [
				...
				localize(
					...
					"Open the Welcome page, with content to aid in getting started with Architech and extensions."
				),
				localize(
					{
						comment: [
