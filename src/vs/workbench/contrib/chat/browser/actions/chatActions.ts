...
const OPEN_CHAT_QUOTA_EXCEEDED_DIALOG =
...
abstract class OpenChatGlobalAction extends Action2 {
	constructor(
		overrides: Pick<
			ICommandPaletteOptions,
			"keybinding" | "title" | "id" | "menu"
		>,
		private readonly mode?: IChatMode
	) {
		super({
			...overrides,
	...
	override async run(
	...
	): Promise<(IChatAgentResult & { type?: "confirmation" }) | undefined> {
		...
		if (
			!this.mode ||
			!chatWidget ||
			!isAncestorOfActiveElement(chatWidget.domNode)
		) {
		...
		const switchToMode =
			(opts?.mode ? chatModeService.findModeByName(opts?.mode) : undefined) ??
			this.mode;
		if (switchToMode) {
			await this.handleSwitchToMode(
				switchToMode,
				chatWidget,
				instaService,
				commandService
		...
		if (opts?.modelSelector) {
			const ids = await languageModelService.selectLanguageModels(
				opts.modelSelector,
				false
			);
			...
			if (!id) {
				throw new Error(
					`No language models found matching selector: ${JSON.stringify(
						opts.modelSelector
					)}.`
				);
			}
		...
		if (opts?.attachScreenshot) {
			...
			if (screenshot) {
				chatWidget.attachmentModel.addContext(
					convertBufferToScreenshotVariable(screenshot)
				);
			}
		}
		if (opts?.attachFiles) {
		...
		if (opts?.query) {
			...
			if (!opts.isPartialQuery) {
				...
				await waitForDefaultAgent(
					chatAgentService,
					chatWidget.input.currentModeKind
				);
		...
		if (opts?.blockOnResponse) {
			...
			if (response) {
				...
				return {
					...response.result,
					type: response.isPendingConfirmation.get()
						? "confirmation"
						: undefined,
				};
			}
	...
	private async handleSwitchToMode(
		switchToMode: IChatMode,
		...
		if (switchToMode) {
			...
			const requestCount =
				chatWidget.viewModel?.model.getRequests().length ?? 0;
...
async function waitForDefaultAgent(
...
abstract class ModeOpenChatGlobalAction extends OpenChatGlobalAction {
	constructor(
		mode: IChatMode,
		keybinding?: ICommandPaletteOptions["keybinding"]
	) {
		super(
			{
...
export function registerChatActions() {
	...
	registerAction2(
		class extends ModeOpenChatGlobalAction {
			constructor() {
				super(ChatMode.Ask);
			}
		}
	...
	registerAction2(
		class ToggleChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor) {
				...
				const chatLocation =
					viewDescriptorService.getViewLocationById(ChatViewId);
		...
		class ChatHistoryAction extends Action2 {
			...
			private showLegacyPicker = async (
				...
				const clearChatHistoryButton: IQuickInputButton = {
					...
					tooltip: localize(
						"interactiveSession.history.clear",
				...
				picker.title = localize(
					"interactiveSession.history.title",
			...
			private showIntegratedPicker = async (
				...
				const getPicks = async (
					...
					const allFastPickItems: IChatPickerItem[] = cachedItems.map((i) => {
						const timeAgoStr = fromNowByDay(i.lastMessageDate, true, true);
						const currentLabel = i.isActive
							? localize("currentChatLabel", "current")
							: "";
						const description = currentLabel
							? `${timeAgoStr} • ${currentLabel}`
							: timeAgoStr;
					...
					const fastPickItems = showAllChats
						? allFastPickItems
						: allFastPickItems.slice(0, 5);
					...
					const slowPicks = (async function* (): AsyncGenerator<
						(IQuickPickSeparator | ICodingAgentPickerItem)[]
					> {
						try {
							...
							try {
								...
								for (const provider of providers) {
									const sessions =
										await chatSessionsService.provideChatSessionItems(
											provider.type,
											cancellationToken.token
										);
									providerNSessions.push(
								...
								for (const session of providerNSessions) {
									...
									const ckey = contextKeyService.createKey(
										"chatSessionType",
										session.providerType
									);
									const actions = menuService.getMenuActions(
										MenuId.ChatSessionsMenu,
										contextKeyService
									);
									...
									const buttons = primary.map((action) => ({
										...
										iconClass:
											action.class ||
											ThemeIcon.asClassName(Codicon.symbolClass),
									}));
									...
									const existingIndex = agentPicks.findIndex(
										(pick) => pick.chat.sessionId === sessionContent.id
									);
									if (existingIndex >= 0) {
										agentPicks[existingIndex] = agentPick;
									} else {
										// Respect show limits
										const maxToShow = showAllAgents
				...
				const picker = store.add(
					quickInputService.createQuickPick<
						IChatPickerItem | ICodingAgentPickerItem
					>({ useSeparators: true })
				);
				picker.title =
					showAllChats || showAllAgents
						? localize(
								"interactiveSession.history.titleAll",
								"All Workspace Chat History"
						  )
						: localize(
								"interactiveSession.history.title",
								"Workspace Chat History"
				...
				store.add(
					picker.onDidTriggerItemButton(async (context) => {
						if (context.button === openInEditorButton) {
							...
							const { fast, slow } = await getPicks(
								showAllChats,
								showAllAgents
							);
							picker.items = fast;
						...
						try {
							...
							if (sessionId === "show-more-chats") {
								...
								if (codingAgentItem.session) {
									await this.showChatSessionInEditor(
										codingAgentItem.session.providerType,
										codingAgentItem.session.session,
										editorService
									);
								}
							}
			...
			async run(accessor: ServicesAccessor) {
				...
				if (editingSession) {
					const phrase = localize(
						"switchChat.confirmPhrase",
						"Switching chats will end your current edit session."
					);
					if (
						!(await handleCurrentEditingSession(
							editingSession,
							phrase,
							dialogService
						))
				...
				const allProviders =
					chatSessionsService.getAllChatSessionItemProviders();
				const hasNonLocalProviders = allProviders.some(
					(provider) => provider.chatSessionType !== "local"
				);
			...
			private async showChatSessionInEditor(
				providerType: string,
				session: IChatSessionItem,
				editorService: IEditorService
			) {
	...
	registerAction2(
		class OpenChatEditorInNewWindowAction extends Action2 {
			constructor() {
				super({
					id: `workbench.action.chat.newChatInNewWindow`,
					title: localize2(
						"chatSessions.openNewChatInNewWindow",
	...
	registerAction2(
		class NewChatInSideBarAction extends Action2 {
			constructor() {
				super({
					id: `workbench.action.chat.newChatInSideBar`,
					title: localize2(
						"chatSessions.newChatInSideBar",
	...
	registerAction2(
		class OpenChatInNewEditorGroupAction extends Action2 {
			constructor() {
				super({
					id: "workbench.action.chat.openNewChatToTheSide",
					title: localize2(
						"chat.openNewChatToTheSide.label",
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				const newGroup = editorGroupService.addGroup(
			...
			constructor() {
				super({
					...
					keybinding: [
						...
						{
							when: ContextKeyExpr.and(
								ContextKeyExpr.or(IsWindowsContext, IsLinuxContext),
								ChatContextKeys.inQuickChat.negate()
							),
							primary: KeyMod.CtrlCmd | KeyCode.UpArrow,
							weight: KeybindingWeight.EditorContrib,
					...
					keybinding: [
						{
							...
							when: ContextKeyExpr.and(
								ChatContextKeys.inChatSession,
								ChatContextKeys.inChatInput.negate(),
								ChatContextKeys.inQuickChat.negate()
							),
						},
						{
	...
	registerAction2(
		class ShowExtensionsUsingCopilot extends Action2 {
			constructor() {
				super({
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				extensionsWorkbenchService.openSearch(
					`@feature:${CopilotUsageExtensionFeatureId}`
				);
			}
		}
	);
	...
	registerAction2(
		class ConfigureCopilotCompletions extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class ShowQuotaExceededDialogAction extends Action2 {
			constructor() {
				super({
			...
			override async run(accessor: ServicesAccessor) {
				...
				const chatQuotaExceeded =
					chatEntitlementService.quotas.chat?.percentRemaining === 0;
				const completionsQuotaExceeded =
					chatEntitlementService.quotas.completions?.percentRemaining === 0;
				...
				if (chatEntitlementService.quotas.resetDate) {
					const dateFormatter = chatEntitlementService.quotas.resetDateHasTime
					...
					const quotaResetDate = new Date(
						chatEntitlementService.quotas.resetDate
					);
					message = [
						message,
						localize(
				...
				await dialogService.prompt({
					...
					buttons: [
						{
							label: free
								? localize("upgradePro", "Upgrade to GitHub Copilot Pro")
								: localize("upgradePlan", "Upgrade GitHub Copilot Plan"),
							run: () => {
	...
	registerAction2(
		class ResetTrustedToolsAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class UpdateInstructionsAction extends Action2 {
			constructor() {
				super({
...
Source existing AI conventions from \`**/{.github/copilot-instructions.md,AGENT.md,AGENTS.md,CLAUDE.md,.cursorrules,.windsurfrules,.kodikrules,.cursor/rules/**,.windsurf/rules/**,.kodikrules/**,README.md}\` (do one glob search).
...
Update \`.github/copilot-instructions.md\` for the user, then ask for feedback on any unclear or incomplete sections to iterate.`;
	...
	registerAction2(
		class OpenChatFeatureSettingsAction extends Action2 {
			constructor() {
				super({
...
const defaultChat = {
	...
	completionsMenuCommand:
		product.defaultChatAgent?.completionsMenuCommand ?? "",
};
...
registerAction2(
	class ToggleCopilotControl extends ToggleTitleBarConfigAction {
		constructor() {
			super(
				"chat.commandCenter.enabled",
				localize("toggle.chatControl", "Chat Controls"),
				localize(
					"toggle.chatControlsDescription",
					"Toggle visibility of the Chat Controls in title bar"
				),
				5,
				ContextKeyExpr.and(
					ContextKeyExpr.and(
...
export class CopilotTitleBarMenuRendering
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const disposable = actionViewItemService.register(
			MenuId.CommandCenter,
			MenuId.ChatTitleBarMenu,
			(action, options) => {
...
export async function handleModeSwitch(
	...
	const needToClearEdits =
		...
		(fromMode === ChatModeKind.Edit || toMode === ChatModeKind.Edit) &&
		requestCount > 0;
	if (needToClearEdits) {
		...
		const undecidedEdits = currentEdits.filter(
			(edit) => edit.state.get() === ModifiedFileEntryState.Modified
		);
...
{
	function registerGenerateCodeCommand(
		coreCommand: string,
		...
		CommandsRegistry.registerCommand(coreCommand, async (accessor) => {
			...
			if (editorGroupService.activeGroup.activeEditor) {
				...
				editorGroupService.activeGroup.pinEditor(
					editorGroupService.activeGroup.activeEditor
				);
			}
...
registerAction2(
	class ToggleDefaultVisibilityAction extends Action2 {
		constructor() {
			super({
				...
				toggled: ContextKeyExpr.equals(
					"config.workbench.secondarySideBar.defaultVisibility",
					"hidden"
				).negate(),
				f1: false,
				menu: {
		...
		async run(accessor: ServicesAccessor) {
			...
			const currentValue = configurationService.getValue<"hidden" | unknown>(
...
registerAction2(
	class EditToolApproval extends Action2 {
		constructor() {
			super({
		...
		async run(accessor: ServicesAccessor, toolId: string): Promise<void> {
			...
			const items: TItem[] = [
				...
				},
				{
					id: "workspace",
					label: localize(
						"chat.toolApproval.workspace",
						"Auto-approve for this workspace"
					),
				},
				{
					id: "profile",
					label: localize("chat.toolApproval.profile", "Auto-approve globally"),
				},
			];

			const quickPick = quickInputService.createQuickPick<TItem>();
			quickPick.placeholder = localize(
				"chat.editToolApproval.title",
				"Approval setting for {0}",
