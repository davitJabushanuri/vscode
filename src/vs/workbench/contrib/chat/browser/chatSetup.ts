...
class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerVSCodeAgent(
		...
		return instantiationService.invokeFunction((accessor) => {
			...
			const { agent, disposable } = SetupAgent.doRegisterAgent(
				instantiationService,
				chatAgentService,
				"setup.vscode",
				"vscode",
				false,
				localize2("vscodeAgentDescription", "Ask questions about Architech")
					.value,
			...
			disposables.add(
				SetupTool.registerTool(instantiationService, {
					...
					modelDescription: localize(
						...
						"Scaffold a new workspace in Architech"
					),
...
class SetupTool extends Disposable implements IToolImpl {
	static registerTool(
		...
		return instantiationService.invokeFunction((accessor) => {
			...
			disposables.add(
				toolService.registerToolImplementation(toolData.id, tool)
			);
	...
	private registerSetupAgents(
		...
		const updateRegistration = () => {
			if (!context.state.hidden && !context.state.disabled) {
				...
				if (
					!(context.state.installed && !context.state.disabled) &&
					...
					disposables.add(
						SetupAgent.registerVSCodeAgent(
							this.instantiationService,
							context,
							controller
						).disposable
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				if (location === ViewContainerLocation.AuxiliaryBar) {
					const activeContainers = viewsDescriptorService
						...
						.filter(
							(container) =>
								viewsDescriptorService.getViewContainerModel(container)
...
class ChatSetupController extends Disposable {
	...
	private async install(
		...
		providerId: string,
		watch: StopWatch,
		options: { useSocialProvider?: string; useEnterpriseProvider?: boolean }
	): Promise<ChatSetupResultValue> {
		const wasRunning =
