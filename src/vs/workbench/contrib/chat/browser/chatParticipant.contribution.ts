...
const chatViewContainer: ViewContainer = Registry.as<IViewContainersRegistry>(
...
// Hide chat view from secondary sidebar - keep services active for extensions like Cline
// Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).registerViews(chatViewDescriptor, chatViewContainer);
void chatViewDescriptor;
void chatViewContainer; // suppress unused warning
				...
				properties: {
					...
					disambiguation: {
						...
						items: {
							...
							type: "object",
							defaultSnippets: [
								{ body: { category: "", description: "", examples: [] } },
							],
		...
		chatParticipantExtensionPoint.setHandler((extensions, delta) => {
			for (const extension of delta.added) {
				for (const providerDescriptor of extension.value) {
					...
					if (
						(providerDescriptor.isDefault || providerDescriptor.modes) &&
...
export class ChatCompatibilityNotifier
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const commandButton = `[${showExtensionLabel}](command:${showExtensionsWithIdsCommandId}?${encodeURIComponent(
			JSON.stringify([[this.productService.defaultChatAgent?.chatExtensionId]])
		)})`;
...
class ChatParticipantDataRenderer
	extends Disposable
	implements IExtensionFeatureTableRenderer
{
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		const nonDefaultContributions =
			manifest.contributes?.chatParticipants?.filter((c) => !c.isDefault) ?? [];
