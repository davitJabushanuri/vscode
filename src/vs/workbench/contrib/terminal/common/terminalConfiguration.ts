...
} from "../terminalContribExports.js";
import {
	...
	properties: {
		...
		[TerminalSettingId.DetectLocale]: {
			markdownDescription: localize(
				...
				"Controls whether to detect and set the `$LANG` environment variable to a UTF-8 compliant option since Architech's terminal only supports UTF-8 encoded data coming from the shell."
			),
			type: "string",
			enum: ["auto", "off", "on"],
			markdownEnumDescriptions: [
		...
		[TerminalSettingId.GpuAcceleration]: {
			...
			markdownEnumDescriptions: [
				localize(
					...
					"Let Architech detect which renderer will give the best experience."
				),
				localize(
		...
		[TerminalSettingId.AllowChords]: {
			markdownDescription: localize(
				...
				"Whether or not to allow chord keybindings in the terminal. Note that when this is true and the keystroke results in a chord it will bypass {0}, setting this to false is particularly useful when you want ctrl+k to go to your shell (not Architech).",
				"`#terminal.integrated.commandsToSkipShell#`"
			),
			type: "boolean",
			default: true,
...
export async function registerTerminalConfiguration(
	...
	if (terminalConfiguration.properties) {
		terminalConfiguration.properties[
			TerminalSettingId.FontFamily
