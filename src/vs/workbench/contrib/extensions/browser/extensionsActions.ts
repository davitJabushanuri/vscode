...
export class ButtonWithDropdownExtensionActionViewItem extends ActionWithDropdownActionViewItem {
	...
	protected override updateClass(): void {
		...
		if (this.element && this.dropdownMenuActionViewItem?.element) {
			...
			const isMenuEmpty =
				(<ButtonWithDropDownExtensionAction>this._action).menuActions.length ===
				0;
...
export class InstallAction extends ExtensionAction {
	...
	constructor(
		...
		@IAllowedExtensionsService
		private readonly allowedExtensionsService: IAllowedExtensionsService,
		...
		this._register(
			this.labelService.onDidChangeFormatters(() => this.updateLabel(), this)
		);
	}
			...
			[
				instantiationService.createInstance(InstallAction, {
					installPreReleaseVersion:
						extensionManagementService.preferPreReleases,
				}),
				instantiationService.createInstance(InstallAction, {
					installPreReleaseVersion:
						!extensionManagementService.preferPreReleases,
...
export class InstallingLabelAction extends ExtensionAction {
	...
	update(): void {
		this.class = `${InstallingLabelAction.CLASS}${
			this.extension && this.extension.state === ExtensionState.Installing
				? ""
				: " hide"
		}`;
	}
		...
		if (this.canInstall()) {
			const extensionInOtherServer =
				this.extensionsWorkbenchService.installed.filter(
					(e) =>
						areSameExtensions(e.identifier, this.extension!.identifier) &&
						e.server === this.server
				)[0];
			if (extensionInOtherServer) {
				// Getting installed in other server
				if (
					extensionInOtherServer.state === ExtensionState.Installing &&
...
export class UninstallAction extends ExtensionAction {
	...
	update(): void {
		...
		this.label =
			this.extension.local?.isApplicationScoped &&
			this.userDataProfilesService.profiles.length > 1
				? localize("uninstallAll", "Uninstall (All Profiles)")
				: UninstallAction.UninstallLabel;
		...
		try {
			...
			alert(
				localize(
					...
					"Please reload Kodik to complete the uninstallation of the extension {0}.",
					this.extension.displayName
				)
			);
		} catch (error) {
...
export class UpdateAction extends ExtensionAction {
	...
	private async computeAndUpdateEnablement(): Promise<void> {
		...
		this.enabled =
			canInstall === true && isInstalled && this.extension.outdated;
	...
	override async run(): Promise<any> {
		...
		if (
			this.extension.local?.source === "vsix" &&
			this.extension.local.pinned
		) {
...
export class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
	...
	constructor(
		...
		this._register(
			allowedExtensionsService.onDidChangeAllowedExtensionsConfigValue((e) =>
				this.update()
			)
		);
		this.update();
	...
	async getActionGroups(): Promise<IAction[][]> {
		...
		for (const [group, actions] of contextMenuActionsGroups) {
			if (group === INSTALL_ACTIONS_GROUP) {
				installActions.push(
					...toActions([[group, actions]], this.instantiationService)[0]
				);
...
export class ExtensionEditorManageExtensionAction extends DropDownExtensionAction {
	constructor(
		...
		super(
			...
			"",
			`${ExtensionAction.ICON_ACTION_CLASS} manage ${ThemeIcon.asClassName(
				manageExtensionIcon
			)}`,
			true,
			instantiationService
...
export class TogglePreReleaseExtensionAction extends ExtensionAction {
	...
	override update() {
		...
		if (!this.extension.preRelease) {
			...
			if (
				this.allowedExtensionsService.isAllowed(this.extension.gallery) !== true
			) {
				return;
			}
		}
	...
	override async run(): Promise<any> {
		...
		if (!allVersions.length) {
			await this.dialogService.info(
				localize("no versions", "This extension has no other versions.")
			);
			return;
		}
		...
		const picks = allVersions.map((v, i) => {
			return {
				id: v.version,
				label: v.version,
				description: `${fromNow(new Date(Date.parse(v.date)), true)}${
					v.isPreReleaseVersion
						? ` (${localize("pre-release", "pre-release")})`
						: ""
				}${
					v.version === this.extension?.local?.manifest.version
						? ` (${localize("current", "current")})`
						: ""
				}`,
				ariaLabel: `${
					v.isPreReleaseVersion ? "Pre-Release version" : "Release version"
				} ${v.version}`,
...
export class SetColorThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() =>
					this.workbenchThemeService.setColorTheme(item.id, undefined)
				),
			ignoreFocusLost,
		});
...
export class SetFileIconThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() =>
					this.workbenchThemeService.setFileIconTheme(item.id, undefined)
				),
			ignoreFocusLost,
		});
...
export class SetProductIconThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() =>
					this.workbenchThemeService.setProductIconTheme(item.id, undefined)
				),
			ignoreFocusLost,
		});
...
export abstract class AbstractConfigureRecommendedExtensionsAction extends Action {
	...
	protected openWorkspaceConfigurationFile(
		...
		return this.getOrUpdateWorkspaceConfigurationFile(
			...
			.then((content) =>
				this.getSelectionPosition(content.value.toString(), content.resource, [
					"extensions",
					"recommendations",
				])
			)
...
export class ConfigureWorkspaceRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
	...
	constructor(
		...
		this._register(
			this.contextService.onDidChangeWorkbenchState(() => this.update(), this)
		);
		this.update();
	}
	...
	implements IExtensionContainer
{
	...
	private computeLabel(): string | null {
		...
		const canAddExtension = () => {
			const runningExtension = this.extensionService.extensions.filter((e) =>
				areSameExtensions(
					{ id: e.identifier.value, uuid: e.uuid },
					this.extension!.identifier
				)
...
export class ToggleSyncExtensionAction extends DropDownExtensionAction {
	...
	} extension-sync ${ThemeIcon.asClassName(syncIgnoredIcon)}`;
	private static readonly SYNC_CLASS = `${
		this.ICON_ACTION_CLASS
	} extension-sync ${ThemeIcon.asClassName(syncEnabledIcon)}`;
	...
	constructor(
		...
		this._register(
			Event.filter(this.configurationService.onDidChangeConfiguration, (e) =>
				e.affectsConfiguration("settingsSync.ignoredExtensions")
			)(() => this.update())
		);
		this._register(
			userDataSyncEnablementService.onDidChangeEnablement(() => this.update())
		);
		this.update();
	}
...
export class ExtensionStatusAction extends ExtensionAction {
	...
	private async computeAndUpdateStatus(): Promise<void> {
			...
			if (this.extension.deprecationInfo.extension) {
				const link = `[${
					...
					`command:extension.open?${encodeURIComponent(
						JSON.stringify([this.extension.deprecationInfo.extension.id])
					)}`
				)})`;
				this.updateStatus(
					{
				...
				const link = `[${localize("settings", "settings")}](${URI.parse(
					`command:workbench.action.openSettings?${encodeURIComponent(
						JSON.stringify([
							this.extension.deprecationInfo.settings
				...
				markdown.appendMarkdown(
					localize(
						...
						this.extension.hasChangelog()
							...
							: URI.parse(
									`command:extension.open?${encodeURIComponent(
										JSON.stringify([this.extension.identifier.id])
									)}`
							  ).toString()
					)
				);
		...
		if (
			...
			const result = await this.extensionsWorkbenchService.canInstall(
				this.extension
			);
			if (result !== true) {
		...
		if (
			this.extension.enablementState === EnablementState.DisabledByAllowlist
		) {
			const result = this.allowedExtensionsService.isAllowed(
				this.extension.local
			);
			if (result !== true) {
				this.updateStatus(
		...
		if (
			this.extension.enablementState === EnablementState.EnabledByEnvironment
		) {
			this.updateStatus(
				{
					message: new MarkdownString(
		...
		if (
			this.extension.enablementState === EnablementState.DisabledByExtensionKind
		) {
			if (
				...
				if (
					this.extensionManagementServerService
						.localExtensionManagementServer === this.extension.server
				) {
					if (
						this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(
							this.extension.local.manifest
				...
				// Extension on Remote Server
					...
					this.extensionManagementServerService
						.remoteExtensionManagementServer === this.extension.server
				) {
					if (
						this.extensionManifestPropertiesService.prefersExecuteOnUI(
							this.extension.local.manifest
		...
		for (const feature of features) {
			...
			const manageAccessLink = `[${localize(
				"manage access",
				"Manage Access"
			)}](${URI.parse(
				`command:extension.open?${encodeURIComponent(
	...
	private updateStatus(
		...
		if (updateClass) {
			if (status?.icon === errorIcon) {
				this.class = `${
					ExtensionStatusAction.CLASS
				} extension-status-error ${ThemeIcon.asClassName(errorIcon)}`;
			} else if (status?.icon === warningIcon) {
				this.class = `${
					ExtensionStatusAction.CLASS
				} extension-status-warning ${ThemeIcon.asClassName(warningIcon)}`;
			} else if (status?.icon === infoIcon) {
				this.class = `${
					ExtensionStatusAction.CLASS
				} extension-status-info ${ThemeIcon.asClassName(infoIcon)}`;
			} else if (status?.icon === trustIcon) {
				this.class = `${ExtensionStatusAction.CLASS} ${ThemeIcon.asClassName(
					trustIcon
				)}`;
			} else {
		...
		if (selectedItems.length) {
			...
			if (localExtensionsToInstall.length) {
				...
				this.notificationService.info(
					localize("finished installing", "Successfully installed extensions.")
				);
			}
		}
	}
...
registerThemingParticipant(
	(theme: IColorTheme, collector: ICssStyleCollector) => {
		...
		if (errorColor) {
			collector.addRule(
				`.extension-editor .header .actions-status-container > .status ${ThemeIcon.asCSSSelector(
					errorIcon
				)} { color: ${errorColor}; }`
			);
			collector.addRule(
				`.extension-editor .body .subcontent .runtime-status ${ThemeIcon.asCSSSelector(
					errorIcon
				)} { color: ${errorColor}; }`
			);
			collector.addRule(
