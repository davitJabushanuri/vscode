...
import {
	...
	CONTEXT_EXTENSIONS_GALLERY_STATUS,
	CONTEXT_HAS_GALLERY,
	DefaultViewsContext,
	ExtensionEditorTab,
...
} from "../common/searchExtensionsTool.js";
...
export const VIEW_CONTAINER = Registry.as<IViewContainersRegistry>(
...
const CONTEXT_GALLERY_SORT_CAPABILITIES = new RawContextKey<string>(
	"gallerySortCapabilities",
	""
);
const CONTEXT_GALLERY_FILTER_CAPABILITIES = new RawContextKey<string>(
	"galleryFilterCapabilities",
	""
);
const CONTEXT_GALLERY_ALL_PUBLIC_REPOSITORY_SIGNED = new RawContextKey<boolean>(
	"galleryAllPublicRepositorySigned",
	false
);
const CONTEXT_GALLERY_ALL_PRIVATE_REPOSITORY_SIGNED =
	new RawContextKey<boolean>("galleryAllPrivateRepositorySigned", false);
const CONTEXT_GALLERY_HAS_EXTENSION_LINK = new RawContextKey<boolean>(
	"galleryHasExtensionLink",
	false
);
	...
	implements IWorkbenchContribution
{
	constructor(
		...
		this._register(
			extensionGalleryManifestService.onDidChangeExtensionGalleryManifestStatus(
				() => this.updateExtensionGalleryStatusContexts()
			)
		);
		extensionGalleryManifestService
			...
			.then((extensionGalleryManifest) => {
				...
				this._register(
					extensionGalleryManifestService.onDidChangeExtensionGalleryManifest(
	...
	private async updateExtensionGalleryStatusContexts(): Promise<void> {
		CONTEXT_HAS_GALLERY.bindTo(this.contextKeyService).set(
			this.extensionGalleryManifestService.extensionGalleryManifestStatus ===
				ExtensionGalleryManifestStatus.Available
		);
		CONTEXT_EXTENSIONS_GALLERY_STATUS.bindTo(this.contextKeyService).set(
			this.extensionGalleryManifestService.extensionGalleryManifestStatus
		);
	}

	private async updateGalleryCapabilitiesContexts(
		extensionGalleryManifest: IExtensionGalleryManifest | null
	): Promise<void> {
		CONTEXT_GALLERY_SORT_CAPABILITIES.bindTo(this.contextKeyService).set(
			`_${extensionGalleryManifest?.capabilities.extensionQuery.sorting
				?.map((s) => s.name)
				?.join("_")}_UpdateDate_`
		);
		CONTEXT_GALLERY_FILTER_CAPABILITIES.bindTo(this.contextKeyService).set(
			`_${extensionGalleryManifest?.capabilities.extensionQuery.filtering
				?.map((s) => s.name)
				?.join("_")}_`
		);
		CONTEXT_GALLERY_ALL_PUBLIC_REPOSITORY_SIGNED.bindTo(
			this.contextKeyService
		).set(
			!!extensionGalleryManifest?.capabilities?.signing
				?.allPublicRepositorySigned
		);
		CONTEXT_GALLERY_ALL_PRIVATE_REPOSITORY_SIGNED.bindTo(
			this.contextKeyService
		).set(
			!!extensionGalleryManifest?.capabilities?.signing
				?.allPrivateRepositorySigned
		);
		CONTEXT_GALLERY_HAS_EXTENSION_LINK.bindTo(this.contextKeyService).set(
			!!(
				extensionGalleryManifest &&
				getExtensionGalleryManifestResourceUri(
					extensionGalleryManifest,
	...
	private registerGlobalActions(): void {
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@recommended:keymaps "),
		});
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@recommended:languages "),
		});
					...
					notificationService.prompt(
						...
						vsixs.length > 1
							? localize(
									...
									"Completed installing extensions. Please reload Kodik to enable them."
							  )
							: localize(
									"InstallVSIXAction.successReload",
									"Completed installing extension. Please reload Kodik to enable it."
							  ),
						[
							{
		...
		const featuresExtensionsWhenContext = ContextKeyExpr.and(
			CONTEXT_HAS_GALLERY,
			ContextKeyExpr.regex(
				CONTEXT_GALLERY_FILTER_CAPABILITIES.key,
				new RegExp(`_${FilterType.Featured}_`)
			)
		);
		this.registerExtensionAction({
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@recentlyPublished "),
		});
		...
		MenuRegistry.appendMenuItem(extensionsFilterSubMenu, {
			...
			when: ContextKeyExpr.and(
				...
				ContextKeyExpr.regex(
					CONTEXT_GALLERY_FILTER_CAPABILITIES.key,
					new RegExp(`_${FilterType.Category}_`)
				)
			),
			group: "2_categories",
			order: 1,
		...
		this.registerExtensionAction({
			...
			run: () =>
				this.extensionsWorkbenchService.openSearch("@workspaceUnsupported"),
		});
		...
		[
			{
				...
				sortCapability: SortBy.InstallCount,
			},
			{
				id: "rating",
				title: localize("sort by rating", "Rating"),
				precondition: BuiltInExtensionsContext.negate(),
				sortCapability: SortBy.WeightedRating,
			},
			{
				id: "name",
				title: localize("sort by name", "Name"),
				precondition: BuiltInExtensionsContext.negate(),
				sortCapability: SortBy.Title,
			},
			{
				id: "publishedDate",
				title: localize("sort by published date", "Published Date"),
				precondition: BuiltInExtensionsContext.negate(),
				sortCapability: SortBy.PublishedDate,
			},
			{
				id: "updateDate",
			...
			const sortCapabilityContext = ContextKeyExpr.regex(
				CONTEXT_GALLERY_SORT_CAPABILITIES.key,
				new RegExp(`_${sortCapability}_`)
			);
			this.registerExtensionAction({
				...
				precondition: ContextKeyExpr.and(
					precondition,
					ContextKeyExpr.regex(
						ExtensionsSearchValueContext.key,
						/^@feature:/
					).negate(),
					sortCapabilityContext
				),
				menu: [
					{
				...
				run: async () => {
					...
					)?.getViewPaneContainer() as IExtensionsViewPaneContainer | undefined;
	...
	private registerContextMenuActions(): void {
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				...
				if (extension) {
					const action =
						instantiationService.createInstance(SetColorThemeAction);
		...
		this.registerExtensionAction({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.has("isPreReleaseExtensionAllowed"),
		...
		this.registerExtensionAction({
			...
			precondition: ContextKeyExpr.and(
				...
				ContextKeyExpr.has("isExtensionAllowed")
			),
			menu: {
				id: MenuId.ExtensionContext,
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.or(
						...
						ContextKeyExpr.and(
							CONTEXT_GALLERY_ALL_PRIVATE_REPOSITORY_SIGNED,
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				...
				if (extension) {
					const action = instantiationService.createInstance(InstallAction, {
						installPreReleaseVersion:
							this.extensionManagementService.preferPreReleases,
					});
					action.extension = extension;
					return action.run();
				}
		...
		this.registerExtensionAction({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					ContextKeyExpr.has("isGalleryExtension"),
					CONTEXT_GALLERY_HAS_EXTENSION_LINK
				),
			},
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				accessor
					.get(IExtensionsWorkbenchService)
					.downloadVSIX(extensionId, "release");
			},
		});
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, extensionId: string) => {
				accessor
					.get(IExtensionsWorkbenchService)
					.downloadVSIX(extensionId, "prerelease");
			},
		});
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor) => {
				...
				const trustedPublishers =
					extensionManagementService.getTrustedPublishers();
				...
				if (result) {
					...
					trustedPublishers.filter(
						(publisher) => !result.some((r) => r.id === publisher.publisher)
					);
			...
			for (let index = 0; index < menus.length; index++) {
				...
				if (menuTitle) {
					menusWithTitles.push({
						...
						item: {
							...menu,
							command: { id: extensionActionOptions.id, title: menuTitle },
						},
					});
				} else {
	...
	constructor(
		...
		if (
			...
			for (const profile of userDataProfilesService.profiles) {
				extensionManagementService
					.getInstalled(ExtensionType.User, profile.extensionsResource)
					.then(async (extensions) => {
						...
						for (const extension of extensions) {
							...
							if (
								productService.trustedExtensionPublishers?.includes(
									publisher
								) ||
								(extension.publisherDisplayName &&
									productService.trustedExtensionPublishers?.includes(
						...
						if (trustedPublishers.size) {
							extensionManagementService.trustPublishers(
								...trustedPublishers.values()
							);
						}
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		super();
		const searchExtensionsTool =
			instantiationService.createInstance(SearchExtensionsTool);
		this._register(
			toolsService.registerTool(SearchExtensionsToolData, searchExtensionsTool)
		);
	}
}
...
registerAction2(
	class ExtensionsGallerySignInAction extends Action2 {
		constructor() {
			super({
		...
		run(accessor: ServicesAccessor): Promise<void> {
			return accessor
				.get(ICommandService)
				.executeCommand(DEFAULT_ACCOUNT_SIGN_IN_COMMAND);
		}
	}
);
