			...
			description: localize(
				{
					...
					comment: [
						"Contribution refers to those that an extension contributes to Architech through an extension/contribution point. ",
					],
				},
...
const viewDescriptor: IJSONSchema = {
	...
	defaultSnippets: [
		{ body: { id: "${1:id}", name: "${2:name}", icon: "${3:icon}" } },
	],
	properties: {
		type: {
...
class ViewsExtensionHandler implements IWorkbenchContribution {
	...
	private removeCustomViewContainers(
		...
		const removedExtensions: ExtensionIdentifierSet = extensionPoints.reduce(
			(result, e) => {
					...
					if (viewContainer !== existingViewContainer) {
						viewsToMove.push(
							...this.viewsRegistry
								...
								.filter(
									(view) =>
										(view as ICustomViewDescriptor).originalContainerId ===
										descriptor.id
								)
						);
					}
		...
		for (const extension of extensions) {
			...
			Object.entries(value).forEach(([key, value]) => {
				...
				for (let index = 0; index < value.length; index++) {
					...
					if (!type) {
						collector.error(
							localize("unknownViewType", "Unknown view type `{0}`.", item.type)
						);
						continue;
					}
