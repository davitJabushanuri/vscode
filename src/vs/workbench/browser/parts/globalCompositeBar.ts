								...
								actions.unshift(
									...[
										toAction({
											...
											run: () =>
												setAccountsActionVisible(storageService, false),
										}),
										new Separator(),
									]
								);
...
abstract class AbstractGlobalActivityActionViewItem extends CompositeBarActionViewItem {
	...
	private updateItemActivity(): void {
		(this.action as CompositeBarAction).activities =
			this.activityService.getActivity(this.compositeBarActionItem.id);
	}
					...
					if (account.canSignOut) {
						providerSubMenuActions.push(
							toAction({
								...
								enabled: true,
								run: () =>
									this.commandService.executeCommand("_signOutOfAccount", {
										providerId,
