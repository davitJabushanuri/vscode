...
// Get the fully qualified name for a proto type, e.g. getFqn('StringRequest') returns 'kodik.StringRequest'
...
export async function loadServicesFromProtoDescriptor() {
	...
	for (const [name, def] of Object.entries(proto.kodik)) {
		if (def && "service" in def) {
			protobusServices[name] = def
		} else {
			addTypeNameToFqn(name, `proto.kodik.${name}`)
		}
	}
